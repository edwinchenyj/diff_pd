...
template<int vertex_dim, int element_dim>
const real HydrodynamicsStateForce<vertex_dim, element_dim>::CdDerivative(const real angle) const {
    const real angle_normalized = angle / (Pi() / 2);
    // angle_normalized now becomes [-1, 1].
    real t = angle_normalized;
    real sign = 1;
...
template<int vertex_dim, int element_dim>
const real HydrodynamicsStateForce<vertex_dim, element_dim>::CtDerivative(const real angle) const {
    if (angle * 180.0 / Pi() > 85.0) return 0.0;
    const real t = angle / (Pi() / 2) * 0.5 + 0.5;
    const real dt = 1 / Pi();
...
void HydrodynamicsStateForce<3, 4>::BackwardForce(const VectorXr& q, const VectorXr& v, const VectorXr& f,
    ...
    for (int i = 0; i < surface_face_num_; ++i) {
        ...
        const Matrix3r df_drag_dv = 0.5 * rho_ * A * (-I * Cd_phi * v_rel_len / 3 + v_rel * Cd_derivative * dphi_dv * v_rel_len
