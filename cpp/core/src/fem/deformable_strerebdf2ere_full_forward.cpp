...
void Deformable<vertex_dim, element_dim>::ForwardSTREREBDF2EREFULL(const std::string& method,
    ...
    const std::map<std::string, real>& options, VectorXr& q_next, VectorXr& v_next, std::vector<int>& active_contact_idx) const {
     ...
     std::map<std::string, real> opt_si = options;
       opt_si["si_method"] = 1;
        ...
        ForwardTREREBDF2EREFULL(method, q, v, a, f_ext, dt, opt_si, q_next, v_next, active_contact_idx);
    }
