...
void Deformable<vertex_dim, element_dim>::ForwardSBDF2EREFULL(const std::string& method,
    ...
    const std::map<std::string, real>& options, VectorXr& q_next, VectorXr& v_next, std::vector<int>& active_contact_idx) const {
        ...
        if (verbose_level > 1) std::cout<<"forward sbdf 2 ere full\n";
        ...
        for (int contact_iter = 0; contact_iter < max_contact_iter; ++contact_iter) {
            ...
            {
                if (verbose_level > 1) std::cout<<"SBDF2ERE newton iteration "<<i<<"\n";
                ...
                if (verbose_level > 1) Toc("SBDF2ERE FULL: decomposition");
...
// q_sol = q;
                ...
                // semi-implicit, only need one iteration
                break;
            }
            q_prev = q;
            v_prev = v;
