...
void Deformable<vertex_dim, element_dim>::ForwardSTRBDF2FULL(const std::string& method,
    ...
    const std::map<std::string, real>& options, VectorXr& q_next, VectorXr& v_next, std::vector<int>& active_contact_idx) const {
       ...
       opt_si["si_method"] = 1;
         ...
         ForwardTRBDF2FULL(method, q, v, a, f_ext, dt, opt_si, q_next, v_next, active_contact_idx);
    }
