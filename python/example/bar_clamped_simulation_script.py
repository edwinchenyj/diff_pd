...
from py_diff_pd.env.bar_clamped_env_3d import BarClampedEnv3d
...
seed = 42
np.random.seed(seed)
folder = Path('bar_clamped')
youngs_modulus = args.Y 
poissons_ratio = args.P 
env = BarClampedEnv3d(seed, folder, { 'youngs_modulus': youngs_modulus,
...
method = args.method
opt = { 'max_pd_iter': 500, 'max_ls_iter': 10, 'abs_tol': 1e-9, 'rel_tol': 1e-4, 'verbose': 2, 'thread_ct': thread_ct,
...
# Generate siere data.
print(f'{folder}/{simulation_name}/output') 
