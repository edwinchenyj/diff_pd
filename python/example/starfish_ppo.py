...
from py_diff_pd.common.rl_sim import DiffPDTask, make_starfish_3d, tensor, MyGaussianActorCriticNet, get_logger, MeanStdNormalizer, AdaSim, IndSim
...
def ppo_ada():
    ...
    folder = Path('starfish').resolve() / 'Ada_PPO'
    ...
    config.task_fn = lambda: DiffPDTask(make_starfish_3d, AdaSim, seed, config.num_workers, False) # pylint: disable=no-member
