...
if __name__ == '__main__':
    ...
    youngs_modulus = 2e5
    poissons_ratio = 0.4
    env = BouncingTetEnv3d(seed, folder, { 'youngs_modulus': youngs_modulus,
    ...
    methods = ('newton_pcg', 'newton_cholesky', 'pd_pardiso','sibe')
    ...
    q_gt = ndarray([0.0, 0.0, 0.0])
    v_gt = ndarray([0.5, 0.5, -0.2])
    ...
    env.simulate(dt, frame_num, methods[3], opts[2], q0, v0, a0, f0, require_grad=False, vis_folder='groundtruth')
    ...
    # E = np.exp(x_init[0])
    # nu = np.exp(x_init[1])
    # env_opt = BouncingArmaEnv3d(seed, folder, { 'youngs_modulus': E, 'poissons_ratio': nu })
